function [ES_Y,iter_time,re_list,total_time,a,S,C,B] = JTKM_iTHALS(Y,M,R,ROU,mu,gama,num_center,maxiter)
...
%                     b=sort(B{n}(:,r));
%                     b=b(ceil(0.9*sz(n)));
%                     B{n}(B{n}(:,r)<=b&B{n}(:,r)<1e-2,r)=-1e-14;
%                 end
%             end
        else
            for r=1:R
                U{n}(:,r)=((ROU(1)+1)*U{n}(:,r)+T2(:,r)-U{n}*T3(:,r)+gama*T4(:,r))/(ROU(1)+1+gama);
                 U{n}(:,r)=U{n}(:,r)+w*(U{n}(:,r)-oldU{n}(:,r));
                B{n}(:,r)=D{n}*U{n}(:,r);
                B{n}((B{n}(:,r)<0),r)=-1e-14;
%                 
